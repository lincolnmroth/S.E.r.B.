...
//							candidateMarkers.lastElement().add(new Point(points[6],points[7]));
		...
		for(int i=0;i<nCandidates;i++){
			if(toRemove.get(i) == 0){
				...
				if(marker.checkBorder()){
                    ...
                    if(marker.getMarkerId()== MarkerTracker.tagID){
                        MarkerTracker.helmetOn=true;
                        Log.i("TRACKER","HELMET DETECTED OPENCV");
                    }
                    //Log.i("MARKER ID:",Integer.toString(marker.getMarkerId()));
                }
			}
		}
        ...
        //for(int i=0;i<newMarkers.size();i++){
            //Log.i("MARKER ID:",Integer.toString(newMarkers.get(i).getMarkerId()));
        //}
	}
